...
export default function LightCard() {
  ...
  useEffect(() => {
    const socket = io("http://localhost:3000", {
      transports: ["websocket"], // Chỉ sử dụng WebSocket, bỏ polling
    });
    ...
    socket.on("adafruit-data", (message) => {
      console.log("Received adafruit-data:", message);
      if (message.topic === "Cloudy1904/feeds/Yolo_Light") {
        const value = parseFloat(message.data);
        setLightStatus(value > 0 ? "online" : "offline");
      }
    });
  ...
  async function toggleLight() {
    dispatch(appearSpinner());
    const newStatus = lightStatus === "online" ? "offline" : "online";
    const result = await toggleDevice("df9c024d-87a0-4363-97d7-914b8c30b18e", {
      status: newStatus,
    });
