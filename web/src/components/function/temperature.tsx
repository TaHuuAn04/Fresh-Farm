      ...
      async function firstFetchTemp() {
        ...
        if(light?.statusCode >= 400) {
          ...
          if(light?.data?.last_value > '28') {
            ...
            setTemperatureData({
              label: "Cold", color: "text-blue-500"
            })
          }
    ...
    <Card>
      <CardHeader className="pb-2">
        <CardTitle className="flex items-center gap-2">
          ...
          <Badge className={`${temperatureData.color}`}>{temperatureData.label}</Badge>
        </CardTitle>
        <CardDescription>Environmental temperature</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="flex flex-col items-center md:mt-3 mt-7">
          <div className="text-6xl font-bold mb-4">{temperatureState || 24}°C</div>
        </div>
      </CardContent>
      <CardFooter className="flex justify-between">
        <div className="text-sm text-muted-foreground mt-7">Range: 15°C - 35°C</div>
      </CardFooter>
    </Card>
  )
}
